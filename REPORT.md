...
| 데모 | `python app.py` → http://127.0.0.1:8000 · 공개 주소 https://senior.dodami-ai.com (집에 둔 공기계 스마트폰에서 늘 켜 둠 — 5항목) |
...
- **직접 물어본 결과는 물어본 사람만** — 늘 켜 두는 공개 주소라, 방문자의 질문과 결과를 지난 기록 목록에 싣지 않는다.
  따로 된 기록 파일(`output/runs_live.jsonl`, 저장소에는 올리지 않음)에 남기고, 끝날 때 받은 무작위 번호표로만 결과와 🔊 가 열린다
- **공개** — https://senior.dodami-ai.com (Cloudflare Tunnel). 공개 전에 `.env` · 소스 · 기록 파일이 밖에서 안 열리는지,
  키 없는 라이브 요청이 거절되는지 확인했다

### 공개 서버 — 노트북에서 공기계 스마트폰으로

**처음 구조** — 노트북에서 서버를 돌리고 Cloudflare 터널로 공개했다. 터널은 서버가 있는 기기가 밖으로 길을 걸어 두는 방식이라,
**노트북이 켜져 있어야만** 평가자와 방문자가 데모를 볼 수 있었다. 노트북에 자동 시작(작업 스케줄러 · 죽으면 10초 뒤 다시)까지
붙였지만, 개인 작업용 노트북이라 2026-09-24 하루에만 예기치 않은 종료 2번 · 다시 시작 2번 · 전원 끄기 1번이 있었고,
그동안 **적어도 6시간 40분** 공개 주소가 열리지 않았다 (윈도우 시스템 기록). 절전 · 로그인 · 포트 겹침까지 노트북을 늘 신경 써야 했다.

**바꾼 구조** — 집에 있던 공기계 **갤럭시 A51 5G** (안드로이드 13) 를 서버로 두었다.

| | 어떻게 |
|---|---|
| 실행 | 안드로이드용 리눅스 환경 **Termux** 에 파이썬 · cloudflared 와 과제 패키지 60여 개를 설치 — 안드로이드용 완성본이 없는 C · Rust 패키지 7개는 폰에서 빌드 (8분 26초) |
| 계속 켜짐 | 서비스 관리자 **termux-services(runit)** 가 데모와 터널을 붙잡고 있다가 죽으면 곧바로 다시 띄운다. **Termux:Boot** 부팅 스크립트가 폰이 켜지면 이것을 띄운다 |
| 밖에서 접속 | 폰 전용 터널을 새로 만들고, 평가자 주소 senior.dodami-ai.com 을 그 터널로 옮겼다 — 주소는 그대로 |
| 보안 | 보안 패치가 멈춘 폰이라 공유기 포트는 열지 않고 터널로만 연다. 서버는 여전히 방문자 키만 쓴다 |

**확인한 것 (2026-09-24 ~ 25)** — 폰을 다시 시작하고 Termux 를 열지 않았는데 **34초 뒤** 공개 주소가 다시 열렸다 ·
서버와 터널 프로그램을 일부러 강제로 끝내자 3초 만에 새로 떴다 (시험용 서버로 확인) · 공개 주소로 재생 · 🔊 듣기 · 결과가 되고, `.env` · 소스 · 기록 파일은 404
...
**5. 해 보고 싶던 것을 제출물에 녹였다.** 교육 과제를 이해하는 데 쓴 시간도 중요하지만, 생각만 하고 해 보지 못한 실험을
제출물에 녹여 내고 싶었다. 데모 화면은 먼저 만들어 둔 AI 회의실 방식을 가져왔고, 공개 서버는 노트북에 매달린 구조를
집에 있던 공기계 스마트폰으로 옮겼다 (5항목). 옮길 때도 과제와 같은 순서를 지켰다 — 자료로 먼저 조사하고,
가장 어려운 관문(폰에서 Rust 패키지 빌드)부터 시험하고, 다시 시작하고 강제로 끝내 보며 정말 스스로 뜨는지 확인했다.
...
- **데모** — 한 번 묻는 데 12\~37초 · 키 화면의 「키 확인」 과 「넣기」 가 따로라 헷갈린다
...
→ 4-D 에서는 이 사례 대신 부록 H 의 재료 다섯 개를 읽었고, 이 대목은 따로 판정하지 않았다.
...
→ 4-D 에서는 이 실행 대신 부록 H 의 재료 다섯 개를 읽었고, 위 다섯 가지는 따로 판정하지 않았다.
...
처음엔 장치를 **한 칸**만 적었다. 그런데 이날 지표가 움직인 두 번 모두 엉뚱한 곳을 가리켰다.
...
2. **절겹침률 추가** — 「다른 절과 4어절 이상 똑같은 문장 ÷ 전체 문장」.
   「절끼리 같은 말을 한다」가 이날 세 번(네 절이 같은 말 · 한 줄이 넷 다 같음 · 첫머리 되풀이)
...
### B-3. 「신호와 경보를 가르고, 설명이 한 줄로 안 되는 지표는 버린다」 — ✅ · 재심은 부록 I-2
...
| 절당읽기 | 3.0 · 3.0 · 3.0 · 3.0 | **절예산 3 이 코드로 묶여** 매번 다 씀 (`graph.py` `조사()` 의 `for _ in range(t["예산"])`) — 고장이 아니라 「예산을 다 썼다」는 증거 |
...
반면 이날 글이 나아진 8번은 **전부 예산이 아니라 설계를 고친 것**이었다. 예산은 3 그대로였다.
...
세고 있었다 (`graph.py` 의 `_인용밀도()`). 3단계에서 작성자가 확인한 `--규칙확인` 은 **정의된 함수 이름만**
찾아서 이걸 못 잡았다.

- 이 줄은 재위임 때만 쓰이는데 실제 실행에서 재위임이 한 번도 안 돌았으므로, **이미 잰 값 중 틀어진 것은 없다**
- 고친 것 — `_인용밀도()` 가 `metrics.문장나누기()` 를 쓰게 했다
...
| 경로 시험 (절예산 1) | 1건만 읽어도 대부분 「충분」 이라 신고 — 재위임이 적게 돈다 | 수업 3-N — 모델은 자기 결과를 후하게 신고했다 · 이날 실행 전부 충분 신고 | 재위임절수 |
| R0 vs 확장 | 확장에서만 제도 절이 생긴다. 근거율·격리율은 큰 차이 없다 | 이날 R0 에서 제도 담당이 안 뽑힘 · 근거율 51.9 vs 56.7 (1회) | 역할 목록 + 읽어서 |
...
→ 4-D 에서 이 절을 읽었지만(부록 H-3) 어느 잣대가 맞는지는 정하지 않았다. 두 잣대를 그대로 둘 다 남긴다.
...
**시험** — 이날 실제로 난 모양(네 절 제목이 전부 「당뇨병」)을 가짜 응답기에 일부러 넣었다.
...
- 틀린 6편(확장 T1 의 기본·배정끔·구역끔·절예산4 · R0 T1 · R0 T5)을 `runs.jsonl` 에서
  `output/runs_무효.jsonl` 로 옮기고 사유를 붙였다. 옮기기 전에 사본을 만들었다 (이 파일은 저장소에는 올리지 않았다)
...
**⑦ 재위임 — 84편 전부 재위임 0.** 절제에서 빼기로 한 결정과 맞다. 경로시험으로 따로 본다.

### G-5. 지표로 판정하지 못한 것 — 4-D 에서 읽는다

- ③ 혼자 vs 팀 — 어느 쪽이 나은지
- ④ 역할끔 보고서가 실제로 제도를 덜 썼는지 (「제도 낱말」 이 대략값이므로)
- ⑥ M1 의 「치료 담당:치매관리법」 절이 실제로 제도를 썼는지 (두 잣대 중 어느 쪽이 맞는지) → 정하지 않고 둘 다 남겼다 (부록 E-4)
...
**작성자 결정** — 둘 다 버린다. 계산은 남기고 `metrics.py` 지표표에 「버림」 으로 표시만 한다.
