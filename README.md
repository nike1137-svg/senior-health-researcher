...
**공개 데모** — https://senior.dodami-ai.com — **늘 켜져 있다.** 처음엔 노트북에서 돌려 노트북이 켜져 있을 때만 열렸는데,
집에 있던 공기계 스마트폰(갤럭시 A51 5G)을 서버로 옮겼다 ([REPORT 5항목](REPORT.md#5-데모-설계))
(그래도 안 열리면 아래 2번대로 내 컴퓨터에서 띄우면 된다 — 키 없이 · 0원)

> **왜 옮겼나** — 평가자와 방문자가 데모를 쓰려면 노트북이 켜져 있어야만 하는 구조라, 노트북을 늘 신경 쓰는 일이 많았다.
> 그러다 집에 있던 공기계 스마트폰을 발견해 늘 켜 두는 서버로 바꿨다. 교육 과제를 이해하는 데 쓴 시간도 중요하지만,
> 생각만 하고 해 보지 못한 실험을 제출물에 녹여 내는 것도 좋겠다고 생각해 진행했다.
...
- **결과는 물어본 사람만 본다** — 지난 기록 목록에 싣지 않고, 끝날 때 받은 번호표로만 열린다
